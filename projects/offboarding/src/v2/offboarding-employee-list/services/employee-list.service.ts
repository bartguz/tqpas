...
export class EmployeeListService {
  ...
  readonly filteredEmployees$: Observable<EmployeeListItem[]> = combineLatest([
  ...
  readonly loading$: Observable<boolean> = this.employeeStorageService.loading$;
  ...
  private loadEmployees(): void {
    this.employeeStorageService.setLoading(true);
    this.loadEmployeesProvider
      ...
      .pipe(
        ...
        finalize(() => this.employeeStorageService.setLoading(false))
      )
      .subscribe();
  }

  private createDetailsLink(employee: Employee): string {
    return `/${OffboardingRoutes.Offboarding}-v2/${OffboardingRoutes.Employee}/${employee.id}`;
  }
